...
using namespace std;
using solver::solve; using solver::RealVariable; using solver::ComplexVariable; //why is that?
...
RealVariable  solver::operator*(RealVariable l, RealVariable r) //why the checks?
{
	...
	return RealVariable(l.getA() * r.getC() + r.getA() * l.getC() + l.getB() * r.getB(),
		...
		l.getC() * r.getC()); //not sure about this
}
